...
class Config:
    ...
    def input_file(self) -> Path:
        return Path(f"../AmazonOME/data/input/{self.input_1}")

    @property
    def input_file_de(self) -> Path:
        return Path(f"../AmazonOME/data/output/{self.input_2}")
    ...
    TEMP_FILE: Path  = Path('details_tmp_ome_url.xlsx')
    TEMP_FILE_DE:Path= Path('details_tmp_de.xlsx')
    MIN_DELAY:float = 0.1
    MAX_DELAY:float = 0.3

    OUTPUT_HEADERS:list[str] =["URL","OE/OEM"]
    OUTPUT_HEADERS_DE: list[str] = ["URL", "Title","price","About_table","item specifics",'description_from_the_seller',"seller_feedback",'category','picture','OME']
    ...
    # 图片下载源Excel文件路径（包含picture和OME列的表格）
    PICTURE_EXCEL_SOURCE: str = f'data/output/{output_2}'

    # 图片下载输出目录
    PICTURE_OUTPUT_DIR: str = f'D:/ome_picture/{product_key}'

    # 图片链接列名
    PICTURE_COL: str = 'picture'

    # OME号列名
    PICTURE_OME_COL: str = 'OME'

    # 下载并发线程数
    PICTURE_MAX_WORKERS: int = 5
