...
class PictureDownloader:
    ...
    def __init__(self, config=None):
        ...
        self.oem_col = getattr(config, 'PICTURE_oem_COL', 'oem')
        ...
        self._oem_locks = {}
        self._oem_locks_lock = threading.Lock()
        self._oem_counters = {}
    ...
    def _get_oem_lock(self, oem: str) -> threading.Lock:
        with self._oem_locks_lock:
            if oem not in self._oem_locks:
                self._oem_locks[oem] = threading.Lock()
            return self._oem_locks[oem]

    def _get_next_image_number(self, oem: str) -> int:
        with self._lock:
            self._oem_counters[oem] = self._oem_counters.get(oem, 0) + 1
            return self._oem_counters[oem]
    ...
    def _sanitize_oem(self, oem: str) -> str:
        if pd.isna(oem):
            return "unknown"
        oem_str = str(oem).strip()
        oem_str = re.sub(r'[<>:"/\\|?*]', '_', oem_str)
        return oem_str[:50] if oem_str else "unknown"
    ...
    def _download_single_image(
        ...
        oem_lock = self._get_oem_lock(oem)
        ...
        with oem_lock:
            ...
            if existing:
                ...
                print(f"  [跳过] {oem}/{existing.name} 已存在")
        ...
        for attempt in range(self.retry_times):
            try:
                ...
                if attempt < self.retry_times - 1:
                    ...
                    with self._lock:
                        self.stats['failed'] += 1
                    print(f"  [失败] {oem}/{file_stem} - {error_msg[:50]}")
        ...
        with oem_lock:
            ...
            if existing:
                ...
                with self._lock:
                    self.stats['skipped'] += 1
                rel_path = existing.relative_to(self.output_dir).as_posix()
                print(f"  [跳过] {oem}/{existing.name} 已存在（下载完成后发现）")
                return True, rel_path
            ...
            try:
                ...
                print(f"  [成功] {oem}/{final_path.name} ({file_size}KB)")
    ...
    def _process_row(self, row_data: Dict) -> Dict:
        ...
        folder_name = self._sanitize_oem(oem)
        ...
        for url in urls:
            oem_lock = self._get_oem_lock(folder_name)
            with oem_lock:
    ...
    def run(self, save_excel: bool = True, output_excel_path: Optional[str] = None) -> pd.DataFrame:
        ...
        if self.oem_col not in df.columns:
            raise ValueError(f"未找到列 '{self.oem_col}'")
        ...
        # 更新DataFrame
        # 更新DataFrame（修复索引不匹配问题）
        print("\n[5/5] 更新Excel数据...")
        path_dict = {r['index']: '\n'.join(r['paths']) for r in results}
        df['image_paths'] = df.index.map(path_dict).fillna('')
        # print("\n[5/5] 更新Excel数据...")
        # image_paths_column = []
